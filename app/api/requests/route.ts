...
export async function POST(request: Request) {
  if (!isAuthorized(request, ['supervision', 'deposito'])) return Response.json({ error: 'Not authorized' }, { status: 401 })
...
// Solo Depósito/Admin avanza el estado (pendiente → preparada → entregada).
